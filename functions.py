...
def collide_hit_rect2(one, two):
    return one.hitbox.colliderect(two.hit_rect)
    ...
    for i in range(tiles_vert):
        for j in range(tiles_hor):
            ...
            tileset.append(pg.transform.scale(
                subimg, (int(st.TILESIZE* wh_ratio), st.TILESIZE)))
    return tileset
...
def keyDown(events, game):
    for event in events:
        if event.type == pg.KEYUP:
            if event.key == st.KEY_RIGHT:
                game.player.RightCheck = False
            if event.key == st.KEY_UP:
                game.player.UpCheck = False
            if event.key == st.KEY_LEFT:
                game.player.LeftCheck = False
            if event.key == st.KEY_DOWN:
...
def get_inputs(game):
    '''
    store keyboard inputs in a key map
    '''
    game.keys = {
                'A': False,
                'B': False,
    ...
    if get_keys[st.KEY_RIGHT]:
        game.player.RightCheck = True
    elif get_keys[st.KEY_LEFT]:
        game.player.LeftCheck = True
    if get_keys[st.KEY_DOWN]:
        game.player.DownCheck = True
    elif get_keys[st.KEY_UP]:
        game.player.UpCheck = True
        
    
    get_mkeys = pg.mouse.get_pressed()
    
    key['A'] = get_mkeys == st.KEY_A or key['A']
    key['B'] = game.key_down == st.KEY_B or key['B']
    key['X'] = game.key_down == st.KEY_ENTER or key['X']
    
    key['START'] = get_keys[st.KEY_MENU]
